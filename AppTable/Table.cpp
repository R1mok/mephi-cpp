...
namespace Prog4 {
	...
	std::string Table::getEl() {
		std::string a;
		for (auto i : this->ar) {
			std::string count = std::to_string(i.second->getCount());
			a += "Cipher:  " + i.first + " Name: " + i.second->getName() + " Firm: " + i.second->getFirm() + " Country: " + i.second->getCountry() + " ";
			a += "Count:  " + std::to_string(i.second->getCount()) + " Cost: " + std::to_string(i.second->getCost()) + "\n";
		}
		return a;
	}
